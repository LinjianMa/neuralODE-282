...
class BasicBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride):
        ...
        self.conv1 = nn.Conv2d(
            in_channels, int(
                in_channels * reduction), 1, stride, bias=True)
        self.bn1 = nn.BatchNorm2d(int(in_channels * reduction))
        self.conv2 = nn.Conv2d(int(in_channels * reduction),
                               int(in_channels * reduction * 0.5), 1, 1, bias=True)
        self.bn2 = nn.BatchNorm2d(int(in_channels * reduction * 0.5))
        self.conv3 = nn.Conv2d(int(in_channels *
                                   reduction *
                                   0.5), int(in_channels *
                                             reduction), (1, 3), 1, (0, 1), bias=True)
        self.bn3 = nn.BatchNorm2d(int(in_channels * reduction))
        self.conv4 = nn.Conv2d(int(in_channels *
                                   reduction), int(in_channels *
                                                   reduction), (3, 1), 1, (1, 0), bias=True)
        self.bn4 = nn.BatchNorm2d(int(in_channels * reduction))
        self.conv5 = nn.Conv2d(int(in_channels * reduction),
                               out_channels, 1, 1, bias=True)
...
class SqueezeNext(nn.Module):
    def __init__(self, width_x, blocks, num_classes):
        ...
        self.conv1 = nn.Conv2d(3,
                               int(width_x * self.in_channels),
                               3,
                               1,
                               1,
                               bias=True)     # For Cifar10
        # self.conv1  = nn.Conv2d(3, int(width_x * self.in_channels), 3, 2, 1,
        # bias=True)     # For Tiny-ImageNet
        ...
        self.conv2 = nn.Conv2d(
            int(width_x * self.in_channels), int(width_x * 128), 1, 1, bias=True)
    ...
    def _make_layer(self, num_block, width_x, out_channels, stride):
        ...
        for _stride in strides:
            layers.append(BasicBlock(int(width_x * self.in_channels),
                                     int(width_x * out_channels), _stride))

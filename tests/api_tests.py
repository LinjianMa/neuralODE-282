...
class TestCollectionState(unittest.TestCase):
    ...
    def test_dopri5(self):
        ...
        def tuple_f(t, y): return (f(t, y[0]), f(t, y[1]))
        tuple_y0 = (y0, y0)

        tuple_y = torchdiffeq.odeint(
            tuple_f, tuple_y0, t_points, method='dopri5')
    ...
    def test_dopri5_gradient(self):
        ...
        for i in range(2):
            def func(y0, t_points): return torchdiffeq.odeint(
                tuple_f, (y0, y0), t_points, method='dopri5')[i]
    ...
    def test_adams(self):
        ...
        tuple_y = torchdiffeq.odeint(
            tuple_f, tuple_y0, t_points, method='adams')
        max_error0 = (sol - tuple_y[0]).max()
        max_error1 = (sol - tuple_y[1]).max()
        self.assertLess(max_error0, eps)
        self.assertLess(max_error1, eps)
    ...
    def test_adams_gradient(self):
        ...
        for i in range(2):
            def func(y0, t_points): return torchdiffeq.odeint(
                tuple_f, (y0, y0), t_points, method='adams')[i]
            self.assertTrue(torch.autograd.gradcheck(func, (y0, t_points)))
